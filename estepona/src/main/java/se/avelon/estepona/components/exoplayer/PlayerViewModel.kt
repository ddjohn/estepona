...
class PlayerViewModel : ViewModel() {
    ...
    private fun currentMediaItemTag(): String? =
        _playerState.value?.currentMediaItem?.localConfiguration?.tag as? String
            ...
            _playerState.value?.getMediaItemAt(previousIndex)?.let { previousMediaItem ->
                hashMapVideoStates[previousMediaItem.mediaId]?.let { previousVideoItem ->
                    if (previousVideoItem.duration - previousVideoItem.currentPosition <= 3000) {
                        hashMapVideoStates[previousMediaItem.mediaId] =
                            previousVideoItem.copy(currentPosition = 0)
                    }
                }
            }
        }
    ...
    fun updateCurrentPosition(id: String, position: Long, duration: Long) {
        ...
        hashMapVideoStates[id] =
            hashMapVideoStates[id]?.copy(position, duration) ?: VideoItem(position, duration)
    }
